...
OCR_SLOTS: List[Tuple[Tuple[int,int,int,int], Tuple[int,int], Tuple[int,int]]] = [
    ((326,200,528,266),   (265,268),  (758,811)),
    ((328,348,571,416),   (268,430),  (756,970)),
    ((325,503,548,566),   (273,586),  (755,1116)),
    ((328,653,545,718),   (265,745),  (758,635)),
    ((326,803,563,866),   (263,876),  (760,783)),
    ((328,951,531,1016),  (263,1031), (756,936)),
    ((321,1105,538,1170), (270,1186), (760,1090)),
]
...
SCROLL_LIMIT = 8
SWIPE_DUR_MS = 1200  # vuốt chậm & ổn định; chỉnh theo ý nếu cần
TAP_THIRD = (366, 83)
# ---------------- Logging helper ----------------
def L(wk, msg: str):
    _log(wk, f"[BLESS] {msg}")
...
def run_bless_flow(wk, targets: List[str], log=None, max_scrolls: int = SCROLL_LIMIT) -> List[str]:
    ...
    for scroll_idx in range(0, max_scrolls + 1):
        ...
        L(wk, f"Kéo trang chậm (dur_ms={SWIPE_DUR_MS}) — 446,1256 → 446,191")
